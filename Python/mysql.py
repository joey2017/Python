...
def get_info(all_move):
    for info in all_move:
        ...
        f = open("D:\\Python\\douban.txt","a")   #将上面的信息每一行以按逗号分隔的规律存入本地
        f.write(num+",")
        f.write(name+",")
        f.write(charactor+",")
        f.write(remark+",")
...
if __name__=="__main__":
    if os.path.exists("D:\\Python") == False:            #两个if来判断是否文件路径存在 新建文件夹 删除文件
        os.mkdir("D:\\Python")
    if os.path.exists("D:\\Python\\douban.txt") == True:
            os.remove("D:\\Python\\douban.txt")
    ...
    f = open("D:\\Python\\douban.txt","r")     #打开路径复制其中的数据，以便导入数据库
    while True:
        line = f.readline()
        if line:
            line = line.strip("\n")
            ...
            remark = line[3]
            score = line[4]
            con.execute("insert into t_doubantop(num,name,charactor,remark,score)values(%s,%s,%s,%s,%s)",[num, name, charactor, remark, score])
        else:                                #导入数据库
            break
